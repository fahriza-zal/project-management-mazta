...
/**
 * Live editor for the outgoing transitions of one project status (Opsi B).
...
/** Re-read just the transitions after a mutation so ids stay in sync. */
...
<template>
  <BaseModal
    ...
    :subtitle="
      fromName
        ? `Dari “${fromName}”, project boleh dipindah ke status berikut. Perubahan tersimpan otomatis.`
        : ''
    "
        ...
        >
          <div class="flex items-center justify-between gap-3">
            <div>
              ...
              <p class="text-xs text-slate-400">Project perlu disetujui sebelum pindah ke sini.</p>
            </div>
            <button
              type="button"
              class="relative h-6 w-11 shrink-0 rounded-full transition disabled:opacity-50"
