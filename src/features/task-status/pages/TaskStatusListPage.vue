...
import { PERM } from '@/features/task-status/permissions'
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <h1 class="text-heading">Task Status</h1>
        <p class="text-body mt-1">{{ pagination.count }} task status(es)</p>
      </div>
    ...
    <BaseCard :padded="false">
      ...
      <BaseEmpty
        ...
        description="Create a task status to define the columns a task can move through."
      >
        <template v-if="auth.can(PERM.CREATE)" #action>
          <BaseButton variant="primary" @click="openCreate">
            <PlusIcon class="h-4 w-4" /> Create Task Status
          </BaseButton>
        </template>
      </BaseEmpty>
