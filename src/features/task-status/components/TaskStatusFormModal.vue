...
/**
 * Create/edit modal for a task status definition.
 * Pass `statusId` to edit (detail is fetched on open); omit it to create.
 *
 * Note: getTaskStatusDefinition returns `units` but not companies, so on edit
...
const title = computed(() => (isEdit.value ? 'Edit Task Status' : 'Create Task Status'))
...
<template>
  ...
  >
    ...
    <form v-else id="task-status-form" class="space-y-4" @submit.prevent="onSubmit">
