...
import ProjectRoleFormModal from '@/features/project-role/components/ProjectRoleFormModal.vue'
  ...
  <div class="space-y-6">
    ...
    <BaseCard :padded="false">
      ...
      <BaseTable v-else :columns="columns" :rows="items" :hoverable="false">
        ...
        <template #cell-isWatcher="{ row }">
          <BaseBadge :color="row.isWatcher ? 'info' : 'slate'" size="sm" dot>
            {{ row.isWatcher ? 'Watcher' : 'Assignable' }}
          </BaseBadge>
        </template>
    ...
    <!-- Create / Edit -->
    <ProjectRoleFormModal v-model="modalOpen" :role-id="editingId" @saved="onSaved" />
