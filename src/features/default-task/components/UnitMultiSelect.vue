...
import { useUnitOptions } from '@/features/default-task/composables/useUnitOptions'

/**
 * Searchable multi-select for units.
 * v-model is an array of unit ids (Int[]). Pass `initialUnits` ([{ id, name }])
 * to prefill the selected chips when editing.
 */
const props = defineProps({
  modelValue: { type: Array, default: () => [] },
  initialUnits: { type: Array, default: () => [] },
  label: { type: String, default: 'Units' },
  error: { type: String, default: '' },
})
const emit = defineEmits(['update:modelValue'])

const { options, loading, load, search } = useUnitOptions()

// Track selected units as { id, name } so chips render without re-fetching.
const selected = ref([...props.initialUnits])
...
function toggle(unit) {
  if (isSelected(unit.id)) {
    selected.value = selected.value.filter((u) => u.id !== unit.id)
  } else {
    selected.value.push({ id: unit.id, name: unit.name })
  }
  emitChange()
}
...
<template>
  <div ref="root" class="relative">
    ...
    <div v-if="selected.length" class="mb-2 flex flex-wrap gap-1.5">
      ...
      >
        {{ u.name }}
        <button type="button" class="hover:text-primary-900" @click="remove(u.id)">
    ...
    >
      ...
      <p v-else-if="!options.length" class="px-3 py-2 text-sm text-slate-400">No units found.</p>
      <button
      ...
      >
        {{ unit.name }}
        <CheckIcon v-if="isSelected(unit.id)" class="h-4 w-4 text-primary-600" />
      </button>
    </div>
